...
namespace LaptopStore.Server.Controllers.v1.Catalog
{
    public class OrdersController : BaseApiController<OrdersController>
    {
        ...
        {
            var brands = await _mediator.Send(new GetAllOrdersQuery());
            return Ok(brands);
        }
        ...
        [HttpPost("add-edit")] // Route cụ thể cho AddEditOrderCommand
        ...
        [AllowAnonymous]
        [HttpPost("place-order")]  // Đổi từ "create-order" thành "place-order"
        public async Task<IActionResult> Post(Domain.Entities.Catalog.Order command)
        {
            Console.WriteLine($"Received MethodPayment: {command.MethodPayment}, IsPayment: {command.IsPayment}");

            Console.WriteLine(JsonSerializer.Serialize(command));  // Kiểm tra dữ liệu nhận được
            ...
            var result = await _mediator.Send(addEditOrderCommand);
            return Ok(result);
        }
